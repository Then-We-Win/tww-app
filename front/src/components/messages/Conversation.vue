<template>
  <div>
    <div id="wrap-cnvrstn" v-bind:class="{hidden:!isActive}">
      ...
      <messages-message v-bind:myId="myId" v-bind:thread="currentThread" class="q-pa-md"></messages-message>
  ...
  export default {
    ...
    data() {
      return {
        ...
        currentThread: {
          ...
          msgs: this.getMsgThread(this.selUser.id, myData.sampleData.conversations)
        }
      }
    },
    watch: { 
      selUser: function(newUser, oldUser) {
        ...
        if (typeof newUser.id == 'undefined') {   // can't seem to test for undefined on newUser object
          ...
          this.isActive = true;
          if (this.useApi) {
            this.apiMsgThread(this.myId, newUser.id)
              .then(msgThread => {
    ...
    methods: {
      apiMsgThread(myUserId, withUserId) {
        ...
        let url = this.apiUrl + 'messages/thread/' + myUserId + '/' + withUserId + '/' + limit + '/' + beforeDate
      ...
      sendMessage(msgText) {
        ...
        this.currentThread.msgs = this.getMsgThread(this.selUser.id, this.conversations);
      },
    }
  }
</script>
